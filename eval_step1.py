...
def eval(model, args, loader, task, dir):
    ...
    with torch.no_grad():
        for step, sample in enumerate(loader):
            ...
            # cv2.imwrite(os.path.join(dir, name[0]+'.jpg'), pred_image)
...
def main(args):
    ...
    target_dir = 'results/step1/target'
    os.makedirs(target_dir, exist_ok=True)
    source_dir = 'results/step1/source'
    os.makedirs(source_dir, exist_ok=True)
    eval(model, args, target_loader, 0, target_dir)
    # eval(model, args, val_loader_old, 0, source_dir)
