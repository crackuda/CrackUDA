...
class DownsamplerBlock(nn.Module):
    def __init__(self, ninput, noutput, nb_tasks=1):
        ...
        self.bn_ini = nn.ModuleList([nn.BatchNorm2d(noutput, eps=1e-3) for i in range(nb_tasks)])
...
class non_bottleneck_1d_RAP(nn.Module):
    def __init__(self, chann, dropprob, dilated, nb_tasks=1):
        ...
        self.parallel_conv_1 = nn.ModuleList([nn.Conv2d(chann, chann, kernel_size=1, stride=1, padding=0, bias=True) for i in range(nb_tasks)]) #nb_tasks=1 for 1st time, its only on CS
        self.bns_1 = nn.ModuleList([nn.BatchNorm2d(chann, eps=1e-03) for i in range(nb_tasks)])
        ...
        self.parallel_conv_2 = nn.ModuleList([nn.Conv2d(chann, chann, kernel_size=1, stride=1, padding=0, bias=True) for i in range(nb_tasks)])
        self.bns_2 = nn.ModuleList([nn.BatchNorm2d(chann, eps=1e-03) for i in range(nb_tasks)])
...
class Encoder(nn.Module):
    def __init__(self, nb_tasks=1):
        ...
        for x in range(0, 2):  # 2 times
            self.layers.append(non_bottleneck_1d_RAP(128, 0.3, 2, nb_tasks)) # dropprob for imagenet pretrained encoder is 0.1 not 0.3, here using 0.3 for imagenet pretrained encoder
...
class Net(nn.Module):
    def __init__(self, num_classes = [20], nb_tasks=1, cur_task=0):  # use encoder to pass pretrained encoder
        ...
        # print('hi, inside erfnet_RA_parallel', current_task, nb_tasks) #current_task will be nb_tasks-1 as we are training on the last task in each incremental step
    ...
    def forward(self, input, task):
        global current_task
        current_task = task # chose which branch of forward pass you need based on if training on current dataset or validating on a previous dataset. 
        output_encoder = self.encoder(input)
        output = self.decoder[task].forward(output_encoder)
